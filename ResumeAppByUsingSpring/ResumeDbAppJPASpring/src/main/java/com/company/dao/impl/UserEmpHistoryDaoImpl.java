    ...
    public List<EmploymentHistory> getAllEmpHistoryByUserId(int id) {
        ...
        try {
            ...
            PreparedStatement stmt = connection.prepareStatement(
                    "SELECT " + " u.id," + " em.id as em_id, " + " em.header," + " em.begin_date," + " em.end_date,"
                            + " em.job_description" + " FROM employment_history em " + "  LEFT JOIN user u"
                            + " ON em.user_id = u.id " + " WHERE em.user_id = ? ");
            stmt.setInt(1, id);
            stmt.execute();
